...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	go func() { rf.resetElectionTimerCh <- true }() //5.2, a server remains in follower state as long as it receives valid RPCs from a leader or candidate
}
...
func (rf *Raft) leaderElectionThread() {
	...
	for {
		...
		case <-rf.resetElectionTimerCh:
			...
			// See documentation for timer.Reset

			rf.logf("got something on reset election timer")
			stopped := rf.electionTimer.Stop()

			if stopped {
				rf.logf("case: reset; stopped an active timer")
			} else {
				rf.logf("case: reset; seems like timer was already stopped")
			}
			newDuration := getNewElectionTimerDuration()
			rf.electionTimer.Reset(newDuration)
			rf.logf("reset election timer; new timer should expired at %s", time.Now().Add(newDuration).Format("2006-01-02 15:04:05.000"))
...
func (rf *Raft) sendHeartBeats() {
	for {
		...
		for serverIndex, _ := range rf.peers {
			...
			rf.logf("Sending heartbeat to %d; args: %v", serverIndex, heartbeatArgs)
...
func (rf *Raft) startLeaderElection() {
	...
	rf.mu.Unlock()
	go func() { rf.resetElectionTimerCh <- true }()
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.term = 0
	rf.me = me
	rf.electionTimer = time.NewTimer(getNewElectionTimerDuration())
	rf.resetElectionTimerCh = make(chan bool)
	rf.stopElectionTimerCh = make(chan bool)

	rf.logf("Initialized new raft node %v", rf)
