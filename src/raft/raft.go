...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	rf.resetElectionTimerCh <- true //5.2, a server remains in follower state as long as it receives valid RPCs from a leader or candidate
}
	...
	for {
		...
		case <-rf.resetElectionTimerCh:
			...
			if !rf.electionTimer.Stop() {
				rf.logf("case: reseting; Seems like election timer expired but channel not drained, shouldn't happen")
				<-rf.electionTimer.C
			}
